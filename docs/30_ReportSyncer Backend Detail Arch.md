...
|`DotNetToolkit.Logging`|Deprecated: previously provided `ILogService` abstractions and adapters. Migrate to `Microsoft.Extensions.Logging.ILogger<T>` and let hosts configure Serilog as the MEL provider.|
...
Allowed references (project → may reference):
    ...
    ReportSyncer.Core
        ...
        → Microsoft.Extensions.Logging (use `ILogger<T>`)
        → DotNetToolkit.Database

    ReportSyncer.Console
        → ReportSyncer.Core
        → DotNetToolkit.General
        → Microsoft.Extensions.Logging (hosts create Serilog pipeline)
...
- **Abstraction**
    - `Microsoft.Extensions.Logging.ILogger<T>` is the logging abstraction to be used from `ReportSyncer.Core`.
...
- **Implementation**
  - **Serilog** is the standard logging implementation:
    ...
    - Integration with host logging providers is done in hosts (e.g. Serilog adapter registered via `AddSerilog`).
  - No other logging frameworks (NLog, log4net, etc.) are allowed in production code.

- **Rules**
    - `ReportSyncer.Core` should only depend on `Microsoft.Extensions.Logging.ILogger<T>`.
    - Serilog configuration and sink wiring must live in host projects; Core must not reference Serilog or host-specific logging implementations.
...
4.  Dependency resolution semantics:
	- `IDependencyResolver` operates on the **set of participating tables for the current job**:
            - Concretely, the set of `TableTask` entries in the `SyncConfiguration` for the active job defines the "participating tables".
...
5. **Table Selection, Dependency Validation & Auto-Ordering**
	- The Frontend sends the job definition with:
		- A set of `TableTasks` (user-selected tables), in an arbitrary order.
	- The backend **must not** trust the incoming order as the execution order.
	- **Runtime behavior：**
		- The Orchestrator calls `IDependencyResolver` with:
			1. The subset of Target tables that have a `TableTask` in the current job.
			2. The inspected `TableSchema` / `ForeignKeyRelation` metadata for the Target.
		- `IDependencyResolver` MUST
			- Build a dependency graph only over the **participating tables**
			- Compute a safe delete/insert execution order (DAG) over that subset
			- Validate that, for every selected table, all referenced Target tables are:
			- Either also selected in this job, or
			- Explicitly configured to be ignored for this job (if such a concept exists).
		- If a selected table has a foreign key to a Target table that is **not part** of the job:
			- The resolver MUST signal a fatal planning error (e.g., `SchemaDependencyException`):
				- include the list of missing tables,
				- and the selected tables that depend on them.
			- The Orchestrator MUST treat this as a pre-flight failure:
				- no delete / insert should be executed.
		-  If planning succeeds:
			- The Orchestrator stores the final execution order in the `ExecutionPlan`
			- The final ordered list is exposed back to the Frontend so the UI can show the actual run order.

6. **Work estimation & planning**
...
7. **Execution (`ISyncOrchestrator` + `IDataWriter`)**
...
8. **Observability, history, and progress**
    
    - Throughout pre-flight and execution:
        - Core services emit structured log events via `Microsoft.Extensions.Logging.ILogger<T>` (MEL).
...
| ------------------------------- | --------- | --------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------ |
| ReportSyncer.Core.Configuration | Interface | IConfigurationLoader        | Load YAML configuration into `SyncConfiguration` from a path/stream.                           | DotNetToolkit.General (file helpers), Microsoft.Extensions.Logging (ILogger<T>)                                                                         | 10                 |
| ReportSyncer.Core.Configuration | Class     | ConfigurationLoader         | Implements `IConfigurationLoader` using YamlDotNet (or similar) and translates IO/YAML errors. | IConfigurationLoader, Microsoft.Extensions.Logging (ILogger<T>)                                                                                         | 10                 |
| ReportSyncer.Core.Configuration | Class     | ConfigurationValidator      | Validate `SyncConfiguration` object for structural & business rules.                           | SyncConfiguration models, Microsoft.Extensions.Logging (ILogger<T>)                                                                                     | 10                 |
| ReportSyncer.Core.Schema        | Interface | ISchemaInspector            | Load runtime `TableSchema` / `ColumnSchema` metadata for a given connection & table.           | DotNetToolkit.Database (`IDbContext`), Microsoft.Extensions.Logging (ILogger<T>)                                                                        | 20                 |
...
| ReportSyncer.Core.Schema        | Interface | IDependencyResolver         | Build and query FK-based dependency graphs used for execution ordering.                        | DotNetToolkit.Database, Microsoft.Extensions.Logging (ILogger<T>)                                                                                       | 20                 |
...
| ReportSyncer.Core.Security      | Interface | IPermissionProfiler         | Probe runtime DB permissions for delete/insert/IDENTITY_INSERT using safe test operations.     | DotNetToolkit.Database, Microsoft.Extensions.Logging (ILogger<T>)                                                                                       | 30                 |
...
| ReportSyncer.Core.Security      | Class     | SafetyValidator             | Centralize all runtime safety checks (prod-to-prod, self-sync, delete policies).               | PermissionsProfile, configuration models, Microsoft.Extensions.Logging (ILogger<T>)                                                                     | 30                 |
...
| ReportSyncer.Core.Sync          | Interface | ISyncOrchestrator           | Primary entry point to execute sync jobs; coordinates pre-flight and execution phases.         | PreFlightValidator, IDataWriter, INamedDbContextFactory, ProgressTracker, Microsoft.Extensions.Logging (ILogger<T>)                                      | 40–60              |
...
| ReportSyncer.Core.Sync          | Interface | IDataWriter                 | Provide delete/insert operations for table data using batching and identity insert management. | DotNetToolkit.Database, SqlServerQueryBuilder, IdentityInsertManager, Microsoft.Extensions.Logging (ILogger<T>)                                         | 50                 |
...
| ReportSyncer.Core.Sync          | Class     | IdentityInsertManager       | Manage `IDENTITY_INSERT` ON/OFF scope safely for a table.                                      | DotNetToolkit.Database, Microsoft.Extensions.Logging (ILogger<T>)                                                                                      | 50                 |
...
| ReportSyncer.Core.Observability | Class     | WorkEstimator               | Estimate initial row counts and total work units before execution.                             | DotNetToolkit.Database, INamedDbContextFactory, Microsoft.Extensions.Logging (ILogger<T>)                                                              | 40, 60             |
| ReportSyncer.Core.Observability | Class     | ProgressTracker             | Track rows processed and compute ETA using EMA.                                                | WorkEstimator, Microsoft.Extensions.Logging (ILogger<T>)                                                                                               | 40, 60             |
| ReportSyncer.Core.Observability | Class     | LogHistoryService           | Read structured logs and reconstruct `RunSummary` collection for history views.                | Microsoft.Extensions.Logging (ILogger<T>), history models                                                                                              | 60                 |
...
**Responsibility**
- Input:
  - `IReadOnlyCollection<TableTask>` for the current job (user-selected tables).
  - `IReadOnlyCollection<TableSchema>` and `IReadOnlyCollection<ForeignKeyRelation>` for the Target database.
- Behavior:
  - Build a dependency graph over the **selected tables only**.
  - Compute:
    - `IReadOnlyList<TableTask>` for delete phase (children → parents).
    - `IReadOnlyList<TableTask>` for insert phase (parents → children).
  - Detect cycles and throw a specific exception (e.g., `SchemaCycleException`) with cycle details.
  - Detect **missing upstream tables**:
    - For each selected table, check that all referenced Target tables that must participate in the sync are present in the selected set.
    - If not, throw `SchemaDependencyException` (or similar) including:
      - list of missing tables,
      - list of selected tables that depend on them.
---
...
|ReportSyncer.Core.Observability|Class|LogHistoryService|Implementation of `IHistoryService` that reconstructs history from structured log files (Serilog JSON or equivalent). Knows filesystem layout but not UI concerns. Any parse error is logged and results are best-effort.|`IHistoryService`, Microsoft.Extensions.Logging (ILogger<T>)|Task 60|
|ReportSyncer.Core.Observability|Class|WorkEstimator|Estimates total rows to be touched per table using `SELECT COUNT(*)` with filters. Key methods: `Task<WorkEstimate> EstimateAsync(TableExecutionContext ctx, CancellationToken ct)`. Used by dry-run, safety checks (`confirmLargeDeletePct`), and ETA calculations. Throws `SyncExecutionException` for DB failures.|`IDbContext`, `SqlServerQueryBuilder`, Microsoft.Extensions.Logging (ILogger<T>)|Task 30, 40, 60|
...
|ReportSyncer.Core.Observability|Interface|ILogContextEnricher|Optional abstraction to add common metadata to logs (JobId, TableName, Phase). Used internally by logging integration so that `SyncOrchestrator` and `TableRunner` can keep logging calls simple.|Microsoft.Extensions.Logging (ILogger<T>)|Task 60|
|ReportSyncer.Core.Observability|Class|LogContextEnricher|Default implementation of `ILogContextEnricher`, integrates with `ILogger<T>` or Serilog’s `LogContext`. Ensures consistent structured logs for operations listed in the Minimal Doc test plan (JobStarting, PreFlightChecksPassed, etc.).|Microsoft.Extensions.Logging (ILogger<T>)|Task 60|
