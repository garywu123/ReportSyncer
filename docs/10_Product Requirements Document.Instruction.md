...
### Scenario: Table Dependency Ordering & Missing Parent Tables

- **Given** a Sync Job with multiple enabled Table Tasks, each mapping a Source table to a Target table.
- **And** the user has selected a subset of Target tables to sync in the UI (e.g., 30 tables out of a larger schema).
- **And** at least one selected Target table has a foreign key dependency on another Target table that is not selected.
- **When** the user starts the sync job (with or without using any "Sort by dependency" UI action).
- **Then** the system must build a dependency graph for the selected Target tables and determine a safe execution order.
- **And** if any required parent table is missing from the selection, the system must:
  - Fail the **Pre-Flight** phase before any data modification.
  - Surface a validation error that clearly lists the missing parent table(s) (e.g., "Table `OrderLines` depends on `Orders`, which is not selected in this job.").
- **And** if all dependencies are satisfied, the system must:
  - Reorder the execution plan according to dependency order (Parents → Children for inserts, Children → Parents for deletes).
