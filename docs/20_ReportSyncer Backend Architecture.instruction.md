...
| **Logging**              | `Serilog`                                                      | `ReportSyncer.Core` uses `Microsoft.Extensions.Logging.ILogger<T>` (MEL). Hosts configure Serilog as the MEL provider (e.g. `AddSerilog`) and create the Serilog pipeline in the host bootstrap. Core must not reference Serilog or `DotNetToolkit.Logging`. |
...
| **`DotNetToolkit.Logging`**  | Class Lib | **Shared**   | Deprecated: previously exposed `ILogService` abstraction. Migrate code to use `Microsoft.Extensions.Logging.ILogger<T>` and let hosts register Serilog as the MEL provider.                                                                                      |
